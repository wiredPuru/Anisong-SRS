...
- [ ] **Step 1 - Reproduce single delete** - with `bun run dev` running,
...
- [ ] **Step 2 - `deleteCards(ids)` with transactional row delete** - in
  ...
  Step 1.
- [ ] **Step 3 - Stream-cache cleanup** - in `server/utils/streamCache.ts`,
...
- [ ] **Step 4 - Bulk route** - `server/api/cards.delete.ts` accepts `{ ids }`
...
- `nuxt-app/server/api/cards.delete.ts` - `{ ids }` support, `parseDeleteBody`
...
**Step 1 evidence:** _not yet recorded_
