...
import type { MonthHeatmapCell } from "~/utils/monthHeatmap";
...
function heatmapCellClass(day: ReviewHeatmapDay): string {
  ...
  const max = data.value?.heatmap.maxCount ?? 0;
...
const monthHeatmap = computed(() => {
  ...
  const allDays = data.value.heatmap.weeks.flatMap((week) => week.days);
...
function monthCellClass(cell: MonthHeatmapCell): string {
  ...
  const max = monthHeatmap.value?.maxCount ?? 0;
      ...
      <div v-else-if="data" class="dashboard-grid">
        ...
        <div class="heatmap-panel">
          <div class="panel-header">
            ...
            <div class="heatmap-header-right">
              <span v-if="heatmapView === 'month'" class="panel-subtitle"
                >{{ monthHeatmap?.totalReviews ?? 0 }} review{{
                  (monthHeatmap?.totalReviews ?? 0) === 1 ? "" : "s"
                }}
                this month</span
              >
              <span v-else class="panel-subtitle"
                >{{ data.heatmap.totalReviews }} review{{ data.heatmap.totalReviews === 1 ? "" : "s" }} in the last
                year</span
              >
          ...
          <template v-if="heatmapView === 'month'">
            <p v-if="!monthHeatmap?.totalReviews" class="state state-compact">No reviews yet this month.</p>
          ...
          <template v-else>
            <p v-if="!data.heatmap.totalReviews" class="state state-compact">No reviews yet.</p>
            <div v-else class="heatmap-scroll">
              <div class="heatmap-grid">
                <div v-for="(week, weekIndex) in data.heatmap.weeks" :key="weekIndex" class="heatmap-week">
          ...
          <div
            v-if="heatmapView === 'month' ? monthHeatmap?.totalReviews : data.heatmap.totalReviews"
...
/* Same tab-seg/tab-seg-btn pattern as /decks and /stats - border and glow
   for the active state, never a fill, since ambient mode strips backgrounds
   with !important. Duplicated per-page like those two, not a shared
   component, matching this project's established scoped-style convention. */
.tab-seg {
  display: flex;
  border: 1px solid var(--border);
